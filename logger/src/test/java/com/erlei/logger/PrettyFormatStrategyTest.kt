...
import com.erlei.logger.format.PrettyFormatStrategy
...
class PrettyFormatStrategyTest {
  ...
  private val logStrategy = MockPrintStrategy()
  ...
  fun defaultLog() {
    ...
    formatStrategy.log(LogLine.obtain(DEBUG, null, "message"))
  ...
  @Test
  fun logWithExceedingMethodCount() {
    val formatStrategy = builder
        .methodCount(50)
        .build()
  ...
  fun logWithBigChunk() {
    ...
    formatStrategy.log(LogLine.obtain(DEBUG, null, chunk1.toString() + chunk2.toString()))
  ...
  private class MockPrintStrategy : PrintStrategy {
    override fun print(logLine: LogLine) {
      logItems.add(LogAssert.LogItem(logLine.level, logLine.fulTag, logLine.message))
    }

    internal var logItems: MutableList<LogAssert.LogItem> = ArrayList()
  }

  private fun assertLog(level: Int): LogAssert {
    return assertLog(null, level)
  }

  private fun assertLog(tag: String?, level: Int): LogAssert {
    return LogAssert(logStrategy.logItems, tag, level)
  }
}
